...
const defaultOptions = {
  ...
  ws(opts) {
    return new new WebSocket(opts.host, opts.protocol);
  }
...
async function command(commandArgs, client) {
  return new Promise((resolve, reject) => {
    ...
    client.resultMap[id] = resultMessage => {
      if (resultMessage.success) resolve(resultMessage.result);else reject(new Error(resultMessage.error.message)); // We won't need this callback again once we use it:

      delete client.resultMap[id];
    };

    client.ws.send(client.options.messageSerializer({ ...commandArgs,
      id
...
function messageHandler(client) {
  return wsMessage => {
    const message = client.options.messageParser(wsMessage); // Emit an event for any message of any type:

    if (message.type) client.emitter.emit(message.type, message); // Emit an event for event-type messages:
...
function clientObject(client) {
  return {
    rawClient: client,
    getStates: async () => command({
      type: 'get_states'
    }, client),
    getServices: async () => command({
      type: 'get_services'
    }, client),
    getPanels: async () => command({
      type: 'get_panels'
    }, client),
    getConfig: async () => command({
      type: 'get_config'
    }, client),
...
async function connectAndAuthorize(client, resolveWith) {
  return new Promise((resolve, reject) => {
    ...
    client.on('auth_ok', () => resolve(resolveWith));
    client.on('auth_invalid', msg => reject(new Error(msg.message)));
...
async function createClient(callerOptions = {}) {
